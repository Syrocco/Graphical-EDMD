...
For effective-force measurements between two massive colloids, run with ```--colloid-force 1 --dc <surface_gap>```. This places two colloids with surface-to-surface gap ```dc```, includes ```dc``` in the output filename, and adds ```colloid1Fx colloid1Fy colloid2Fx colloid2Fy``` to the thermo file.
